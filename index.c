...
int index_load(Index *index) {
    ...
    if (!f) {
        // File not existing is fine — empty index
        return 0;
    }
...
int index_save(const Index *index) {
    ...
    // Flush userspace buffers to kernel
    fflush(f);
...
int index_add(Index *index, const char *path) {
    ...
    if (file_size > 0 && fread(file_data, 1, (size_t)file_size, f) != (size_t)file_size) {
        free(file_data);
        fclose(f);
        return -1;
    }
